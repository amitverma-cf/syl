...
This is where the "load a real llama.cpp shared library" verification from
`docs/active/plan.md`'s verification section belongs — it's not a unit test of one function,
it's a contract test of the whole plugin-loading path.

**What must be tested before a task counts as done** (checked in `git-workflow.md` step 4):
...
A task in `docs/active/tasks.md` isn't complete when the code compiles — it's complete when
its **testable end goal** (stated in the task itself) can be demonstrated: a passing test
suite for logic-level goals, or an actual run of the app for UI/end-to-end goals. The PR
description (`git-workflow.md` step 6) states which one applied and how it was verified.
