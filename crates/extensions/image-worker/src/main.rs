...
//! `SdEngine` (unchanged, reused as-is from `crates/native-engines`) and speaks
//! the `image.generate/v1` capability over `extension-host`'s stdio
//! protocol. One-shot, non-streaming — mirrors `chat-worker`'s
//! synchronous, single-request-at-a-time design.
//!
//! Usage: `image-worker --library <path> --model <path> [--n-threads <i32>]`
...
use native_engines::stable_diffusion::SdEngine;
use serde_json::json;
