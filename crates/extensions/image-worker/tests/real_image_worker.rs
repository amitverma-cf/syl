//! End-to-end proof that the real `image-worker` binary, spawned as a real
...
//! `cargo test -p image-worker --test real_image_worker -- --ignored`.
...
async fn generates_a_real_image_through_the_isolated_process() {
    ...
    let manifest = ExtensionManifest {
        ...
        backend: Some(ExtensionBackend {
            command: env!("CARGO_BIN_EXE_image-worker").to_string(),
            args: vec![
                "--library".to_string(),
                library_path,
                "--model".to_string(),
