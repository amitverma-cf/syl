use cloud_providers::CustomProviderConfig;
...
fn list_custom_providers_is_empty_when_file_missing() {
    ...
    assert!(cloud_providers::list_custom_providers(&path).is_empty());
}
...
fn list_custom_providers_reads_a_hand_written_config_file() {
    ...
    let loaded = cloud_providers::list_custom_providers(&path);
...
fn remove_custom_provider_deletes_the_entry_and_its_stored_key() {
    ...
    cloud_providers::set_api_key(&env_path, "CUSTOM_ACME_API_KEY", "sk-acme").unwrap();
    cloud_providers::set_api_key(&env_path, "CUSTOM_OTHER_API_KEY", "sk-other").unwrap();

    cloud_providers::remove_custom_provider(&providers_path, &env_path, "acme").unwrap();

    let remaining = cloud_providers::list_custom_providers(&providers_path);
    assert_eq!(remaining.len(), 1);
    assert_eq!(remaining[0].name, "other");

    let key_providers = cloud_providers::list_providers(&env_path);
    let _ = key_providers; // list_providers only reports known cloud providers, not custom ones
    let entries = cloud_providers::load_env_file(&env_path);
...
fn remove_custom_provider_is_a_no_op_when_the_name_does_not_exist() {
    ...
    cloud_providers::remove_custom_provider(&providers_path, &env_path, "does-not-exist").unwrap();

    assert_eq!(
        cloud_providers::list_custom_providers(&providers_path).len(),
        1
    );
...
fn update_custom_provider_replaces_the_existing_entry_via_a_real_discovery_call() {
    ...
    let updated = cloud_providers::update_custom_provider(
        &providers_path,
        &env_path,
        "acme",
        &base_url,
    ...
    let stored = cloud_providers::list_custom_providers(&providers_path);
...
fn list_all_models_merges_catalog_and_custom_providers() {
    ...
    let models = cloud_providers::list_all_models(&path);
