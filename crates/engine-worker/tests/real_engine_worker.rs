//! End-to-end proof that the real `engine-worker` binary, spawned as a real
...
//! (same requirement as `engine-host`'s `real_llama_generate.rs`) — run
//! manually with `cargo test -p engine-worker --test real_engine_worker -- --ignored`.
...
async fn spawn_real_engine_worker() -> std::sync::Arc<ExtensionProcess> {
...
async fn generates_real_text_and_counts_real_tokens_through_the_isolated_process() {
    let process = spawn_real_engine_worker().await;
