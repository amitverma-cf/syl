...
fn resolve_download_url_finds_existing_local_file() {
    let tmp = std::env::temp_dir().join("syl-extension-registry-url-test.txt");
...
fn resolve_local_path_returns_local_file_directly() {
    let tmp = std::env::temp_dir().join("syl-extension-registry-local-path-test.txt");
    std::fs::write(&tmp, b"test").unwrap();
    let url = format!("file://{}", tmp.to_string_lossy().replace('\\', "/"));
    let cache_dir = std::env::temp_dir().join("syl-extension-registry-cache-unused");
