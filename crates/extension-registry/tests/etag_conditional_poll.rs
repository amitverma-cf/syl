...
use extension_registry::fetch_remote_registry_conditional;
...
fn etag_read_write_round_trips_through_a_real_registry_dir() {
    ...
    assert_eq!(extension_registry::read_etag(&dir, "engines"), None);

    extension_registry::write_etag(&dir, "engines", ENGINES_ETAG).unwrap();
    assert_eq!(
        extension_registry::read_etag(&dir, "engines").as_deref(),
        Some(ENGINES_ETAG)
    );

    // Writing again overwrites, not appends.
    extension_registry::write_etag(&dir, "engines", "\"newer\"").unwrap();
    assert_eq!(
        extension_registry::read_etag(&dir, "engines").as_deref(),
        Some("\"newer\"")
    );
