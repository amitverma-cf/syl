use extension_registry::{DownloadSource, ModelKind};
...
pub fn list_available_models() -> Result<Vec<CatalogModel>, String> {
    ...
    let models =
        extension_registry::load_model_entries(&registry_dir).map_err(|e| e.to_string())?;
    ...
    Ok(models
        .into_iter()
        .map(|m| {
            let already_downloaded = match extension_registry::resolve_download_url(&m.download_url)
            {
                Ok(DownloadSource::Local(_)) => true,
                Ok(DownloadSource::Remote(url)) => extension_registry::is_cached(&url, &models_dir),
                Err(_) => false,
            };
            CatalogModel {
...
pub async fn download_model(name: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let path =
            extension_registry::resolve_model_entry_files(&entry, &workspace_paths::models_dir())
                .map_err(|e| e.to_string())?;
        if let Some(sha256) = &entry.sha256 {
            if let Err(e) = extension_registry::verify_sha256(&path, sha256) {
